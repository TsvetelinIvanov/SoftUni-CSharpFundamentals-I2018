...
namespace MusicShopManager.Engine
{
    public sealed class MusicShopEngine : IMusicShopEngine
    {
        ...
        public void Start()
        {
            ICollection<ICommand> commands = this.ReadCommands();
            IEnumerable<string> commandResults = this.ProcessCommands(commands);
        ...
        private ICollection<ICommand> ReadCommands()
        {
            ...
            foreach (string line in this.userInterface.Input())
            {
            ...
            foreach (ICommand command in commands)
            {
                ...
                switch (command.Name)
                {
                    ...
                    case EngineConstants.CreateMicrophoneCommand:
                        commandResult = this.CreateMicrophone(command.Parameters["make"], command.Parameters["model"], decimal.Parse(command.Parameters["price"]), this.ParseBoolean(command.Parameters["cable"]));
                        break;
                    case EngineConstants.CreateDrumsCommand:
                        commandResult = this.CreateDrums(command.Parameters["make"], command.Parameters["model"], decimal.Parse(command.Parameters["price"]), command.Parameters["color"], int.Parse(command.Parameters["width"]), int.Parse(command.Parameters["height"]));
                        break;
                    case EngineConstants.CreateElectricGuitarCommand:
                        commandResult = this.CreateElectricGuitar(command.Parameters["make"], command.Parameters["model"], decimal.Parse(command.Parameters["price"]), command.Parameters["color"], command.Parameters["body"], command.Parameters["fingerboard"], int.Parse(command.Parameters["adapters"]), int.Parse(command.Parameters["frets"]));
                        break;
                    case EngineConstants.CreateAcousticGuitarCommand:
                        commandResult = this.CreateAcousticGuitar(command.Parameters["make"], command.Parameters["model"], decimal.Parse(command.Parameters["price"]), command.Parameters["color"], command.Parameters["body"], command.Parameters["fingerboard"], this.ParseBoolean(command.Parameters["case"]), command.Parameters["strings"]);
                        break;
                    case EngineConstants.CreateBassGuitarCommand:
                        commandResult = this.CreateBassGuitar(command.Parameters["make"], command.Parameters["model"], decimal.Parse(command.Parameters["price"]), command.Parameters["color"], command.Parameters["body"], command.Parameters["fingerboard"]);
                        break;
                    case EngineConstants.AddArticleToShopCommand:
                        commandResult = this.AddArticleToShop(command.Parameters["name"], command.Parameters["make"], command.Parameters["model"]);
                        break;
                    case EngineConstants.RemoveArticleFromShopCommand:
                        commandResult = this.RemoveArticleFromShop(command.Parameters["name"], command.Parameters["make"], command.Parameters["model"]);
                        break;
                    case EngineConstants.ListArticlesCommand:
                        commandResult = this.ListArticles(command.Parameters["name"]);
                        break;
                    default:
        ...
        private string CreateMusicShop(string name)
        {
            ...
            IMusicShop musicShop = this.musicShopFactory.CreateMusicShop(name);
        ...
        private string CreateMicrophone(string make, string model, decimal price, bool hasCable)
        {
            ...
            IMicrophone microphone = this.articleFactory.CreateMirophone(make, model, price, hasCable);
        ...
        private string CreateDrums(string make, string model, decimal price, string color, int width, int height)
        {
            ...
            IDrums drums = this.articleFactory.CreateDrums(make, model, price, color, width, height);
            this.articles.Add(name, drums);

            return string.Format(EngineConstants.ArticleCreatedMessage, "Drums", name);
        }

        private string CreateElectricGuitar(string make, string model, decimal price, string color, string bodyWood, string fingerboardWood, int numberOfAdapters, int numberOfFrets)
        {
            string name = make + " " + model;
            try
            {
            ...
            IElectricGuitar electricGuitar = this.articleFactory.CreateElectricGuitar(make, model, price, color, bodyWood, fingerboardWood, numberOfAdapters, numberOfFrets);
            this.articles.Add(name, electricGuitar);

            return string.Format(EngineConstants.ArticleCreatedMessage, "Electric guitar", name);
        }

        private string CreateAcousticGuitar(string make, string model, decimal price, string color, string bodyWood, string fingerboardWood, bool caseIncluded, string stringMaterial)
        {
            string name = make + " " + model;
            try
            {
            ...
            IAcousticGuitar acousticGuitar = this.articleFactory.CreateAcousticGuitar(make, model, price, color, bodyWood, fingerboardWood, caseIncluded, (StringMaterial)Enum.Parse(typeof(StringMaterial), stringMaterial));
        ...
        private string CreateBassGuitar(string make, string model, decimal price, string color, string bodyWood, string fingerboardWood)
        {
            ...
            IBassGuitar bassGuitar = this.articleFactory.CreateBassGuitar(make, model, price, color, bodyWood, fingerboardWood);
