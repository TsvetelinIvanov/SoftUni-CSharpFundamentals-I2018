...
namespace MyTunesShop
{
    public class MyTunesEngine
    {
        ...
        public Printer Printer => this.printer;
        ...
        protected virtual void ExecuteInsertPerformerCommand(string[] commandWords)
        {
            ...
            {
                case "singer":
                    Singer singer = new Singer(commandWords[3]);
        ...
        protected virtual void ExecuteInsertMediaCommand(string[] commandWords)
        {
            switch (commandWords[2])
            {
                case "song":
                    ...
                    Song song = new Song(commandWords[3], decimal.Parse(commandWords[4]), performer, commandWords[6], int.Parse(commandWords[7]), commandWords[8]);
        ...
        protected virtual void ExecuteSupplyCommand(string[] commandWords)
        {
            ...
            {
                case "song":
                    IMedia song = this.media.FirstOrDefault(s => s is ISong && s.Title == commandWords[2]);
                    if (song == null)
                    {
        ...
        protected virtual void ExecuteReportPerformerCommand(string[] commandWords)
        {
            ...
            {
                case "singer":
                    Singer singer = this.performers.FirstOrDefault(p => p is Singer && p.Name == commandWords[3]) as Singer;
                    if (singer == null)
                    {
